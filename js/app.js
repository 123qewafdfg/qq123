    ...
    (function() {
        ...
        async function initImageCipherWasm() {
            if (!imageCipherWasmAvailable) {
                logImageCipherDebug('wasm-skip-disabled', { url: IMAGE_CIPHER_WASM_URL });
                return null;
            }
            if (!imageCipherWasmPromise) {
                logImageCipherDebug('wasm-load-start', {
                    url: IMAGE_CIPHER_WASM_URL,
                    page: location.href,
                ...
                imageCipherWasmPromise = import(IMAGE_CIPHER_WASM_URL)
                    .then(async (module) => {
                        ...
                        logImageCipherDebug('wasm-load-success', {
                            url: IMAGE_CIPHER_WASM_URL,
                            exports: Object.keys(module).sort(),
                            methods: module.CipherMethod,
                            modes: module.CipherMode
                        });
                    ...
                    .catch((error) => {
                        ...
                        logImageCipherDebug('wasm-load-failed-fallback-js', {
                            url: IMAGE_CIPHER_WASM_URL,
                            message: error && error.message ? error.message : String(error),
                            stack: error && error.stack ? error.stack : null
                        });
                        console.warn('Image cipher WASM unavailable; falling back to JavaScript processing', error);
                        return null;
                    });
            } else {
                logImageCipherDebug('wasm-load-reuse-promise', { url: IMAGE_CIPHER_WASM_URL });
            }
        ...
        async function processImagePixelsWithWasm(data, width, height, mode, blockW, blockH, keyStr, rounds, shouldApplyXOR) {
            ...
            const result = new Uint8ClampedArray(output);
            logImageCipherDebug('wasm-process-success', Object.assign({}, request, {
        ...
        function startImageCipherSuccessEffects() {
            ...
            logImageCipherDebug('wasm-success-effects-start', {
                gsap: !!window.gsap,
                pixi: !!window.PIXI
            });
        ...
        function startWasmButtonFlipEffect() {
            if (!window.gsap) {
                logImageCipherDebug('gsap-missing-skip-button-flip');
                return;
            }
            ...
            const tl = window.gsap.timeline({
                ...
                onComplete: () => {
                    window.gsap.set(buttons, { scaleX: 1, scaleY: 1 });
                    window.gsap.to(buttons, {
                        '--wasm-effect-opacity': 0.65,
                        '--wasm-glow-soft': 'rgba(255, 220, 91, 0.55)',
                        '--wasm-glow-wide': 'rgba(255, 178, 30, 0.42)',
                        '--wasm-gold-top': 'rgba(255, 237, 154, 0.38)',
                        '--wasm-gold-edge': 'rgba(255, 251, 217, 0.6)',
                        '--wasm-gold-depth': 'rgba(94, 48, 0, 0.16)',
                        '--wasm-gold-text': 'rgba(255, 244, 179, 0.6)',
                        duration: 1,
                        ease: 'power1.out',
                        overwrite: 'auto'
                    });
            ...
            // breath pulse loop — starts after initial glow-up (squash ~0.71s + glow 1s)
            window.gsap.to(buttons, {
                '--wasm-glow-soft': 'rgba(255, 220, 91, 0.28)',
                '--wasm-glow-wide': 'rgba(255, 178, 30, 0.2)',
                '--wasm-gold-text': 'rgba(255, 244, 179, 0.35)',
                duration: 1.8,
                ease: 'sine.inOut',
                yoyo: true,
                repeat: -1,
            ...
            logImageCipherDebug('gsap-button-flip-started', {
                count: buttons.length,
                effect: 'squash-stretch-bounce'
            });
        }

        async function startWasmGoldPixiEffect() {
            if (!window.PIXI) {
                logImageCipherDebug('pixi-missing-skip-gold-effect');
                return;
            }
            try {
                ...
                targets.forEach(button => button.classList.add('wasm-cipher-effect-ready'));
                ...
                app.canvas.className = 'wasm-gold-effects-layer';
                app.canvas.setAttribute('aria-hidden', 'true');
                document.body.appendChild(app.canvas);
                if (window.gsap) {
                    window.gsap.to(app.canvas, {
                        opacity: 1,
                        duration: 1,
                        ease: 'power1.out'
                    });
                ...
                function createButtonSweepTexture() {
                    const canvas = document.createElement('canvas');
                    canvas.width = 300;
                    canvas.height = 220;
                    const ctx = canvas.getContext('2d');
                    const gradient = ctx.createLinearGradient(0, 0, canvas.width, 0);
                    gradient.addColorStop(0, 'rgba(255, 213, 96, 0)');
                    gradient.addColorStop(0.18, 'rgba(255, 199, 52, 0)');
                    gradient.addColorStop(0.34, 'rgba(255, 190, 37, 0.22)');
                    gradient.addColorStop(0.44, 'rgba(255, 224, 97, 0.78)');
                    gradient.addColorStop(0.5, 'rgba(255, 255, 250, 1)');
                    gradient.addColorStop(0.56, 'rgba(255, 229, 113, 0.78)');
                    gradient.addColorStop(0.68, 'rgba(255, 181, 23, 0.24)');
                    gradient.addColorStop(0.82, 'rgba(255, 191, 48, 0)');
                    gradient.addColorStop(1, 'rgba(255, 191, 48, 0)');
                    ...
                    core.addColorStop(0.47, 'rgba(255, 255, 255, 0)');
                    core.addColorStop(0.5, 'rgba(255, 255, 255, 1)');
                    core.addColorStop(0.53, 'rgba(255, 255, 255, 0)');
                ...
                const sweepTexture = createButtonSweepTexture();
                let tick = 0;

                const layers = targets.map((button) => {
                    const buttonLayer = new PIXI.Container();
                    const mask = new PIXI.Graphics();
                    const flash = new PIXI.Sprite(sweepTexture);
                    flash.anchor.set(0.5);
                    flash.blendMode = 'add';
                    flash.rotation = Math.PI / 4;
                    buttonLayer.mask = mask;
                    buttonLayer.addChild(flash);
                    app.stage.addChild(mask, buttonLayer);
                    return { button, buttonLayer, mask, flash };
                });

                app.ticker.add(() => {
                    tick += app.ticker.deltaMS / 1000;
                    layers.forEach((layer, targetIndex) => {
                        const { button, buttonLayer, mask, flash } = layer;
                        const rect = button.getBoundingClientRect();
                        const visible = rect.width > 0 && rect.height > 0;
                        buttonLayer.visible = visible;
                        mask.visible = visible;
                        if (!visible) return;
                        mask.clear();
                        mask.roundRect(rect.left, rect.top, rect.width, rect.height, Math.min(60, rect.height / 2)).fill({ color: 0xffffff, alpha: 1 });

                        const localTick = tick + targetIndex * 0.42;
                        const cycleDuration = 2.65;
                        const sweepDuration = 1.18;
                        const cycleTime = localTick % cycleDuration;
                        const sweeping = cycleTime <= sweepDuration;
                        flash.visible = sweeping;
                        if (!sweeping) return;

                        const progress = cycleTime / sweepDuration;
                        flash.x = rect.left - rect.width * 0.45 + progress * rect.width * 1.9;
                        flash.y = rect.top - rect.height * 0.45 + progress * rect.height * 1.9;
                        flash.width = rect.width * 1.12;
                        flash.height = rect.height * 4.6;
                        flash.alpha = 1;
                    });
                });
                ...
                logImageCipherDebug('pixi-gold-effect-failed', {
                    message: error && error.message ? error.message : String(error),
                    stack: error && error.stack ? error.stack : null
                });
                console.warn('PixiJS gold effect failed', error);
            }
        }
        ...
        function getImageRemapWorker() {
            ...
            try {
                ...
                console.warn('Image remap worker is unavailable', error);
                imageRemapWorkerAvailable = false;
        ...
        async function remapImagePixelsWithGpuFallback(data, pixelMap, width, height) {
            ...
            if (totalPixels >= GPU_REMAP_MIN_PIXELS && totalPixels <= GPU_REMAP_MAX_PACKED_INDEX && imageGpuRemapAvailable !== false) {
                try {
                    ...
                    console.warn('WebGL image remap failed; falling back to CPU', error);
            ...
            if (totalPixels >= WORKER_REMAP_MIN_PIXELS && imageRemapWorkerAvailable) {
                ...
                if (workerResultPromise) {
                    try {
                        ...
                        console.warn('Worker image remap failed; falling back to main thread CPU', error);
                    }
                }
            }
        ...
        async function processImage(file, mode, blockW, blockH, shouldApplyXOR) {
            ...
            if (mode === 'encrypt') {
                ...
                try {
                    ...
                    logImageCipherDebug('single-encrypt-wasm-error-fallback-js', {
                        method: imgMethod,
                        width,
                        height,
                        blockW,
                    ...
                    console.warn('WASM image encrypt failed; falling back to JavaScript processing', error);
                }
                if (!newData) {
                    logImageCipherDebug('single-encrypt-js-fallback-start', {
                        method: imgMethod,
                        width,
                        height,
                        blockW,
                    ...
                    logImageCipherDebug('single-encrypt-js-fallback-finish', {
                        ...
                        width,
                        height,
                        mapLength: encMap.length
                    });
                } else {
                    logImageCipherDebug('single-encrypt-used-wasm', {
                        method: imgMethod,
                        width,
                        height,
                ...
                try {
                    ...
                    logImageCipherDebug('single-decrypt-wasm-error-fallback-js', {
                        method: imgMethod,
                        width,
                        height,
                        blockW,
                    ...
                    console.warn('WASM image decrypt failed; falling back to JavaScript processing', error);
                }
                if (!newData) {
                    logImageCipherDebug('single-decrypt-js-fallback-start', {
                        method: imgMethod,
                        width,
                        height,
                        blockW,
                    ...
                    logImageCipherDebug('single-decrypt-js-fallback-finish', {
                        ...
                        width,
                        height,
                        mapLength: invMap.length
                    });
                } else {
                    logImageCipherDebug('single-decrypt-used-wasm', {
                        method: imgMethod,
                        width,
                        height,
                        bytes: newData.length
        ...
        async function processImageBatchInMemory(file, mode, blockW, blockH, count, shouldApplyXOR, format) {
            ...
            try {
                ...
                logImageCipherDebug('batch-wasm-error-fallback-js', {
                    mode,
                    method: imgMethod,
                    width,
                    height,
                ...
                console.warn('WASM batch image processing failed; falling back to JavaScript processing', error);
            }

            if (wasmData) {
                data = wasmData;
                logImageCipherDebug('batch-used-wasm', {
                    mode,
                    method: imgMethod,
                    width,
                    height,
                ...
                logImageCipherDebug('batch-js-fallback-start', {
                    mode,
                    method: imgMethod,
                    width,
                    height,
                ...
                logImageCipherDebug('batch-js-fallback-finish', {
                    mode,
                    method: imgMethod,
                    width,
                    height,
